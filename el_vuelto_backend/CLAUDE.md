...
2. Aggregates quantities **per product** (a client may repeat a `product` across lines) — this is what the rest of `create()` prices and decrements. It no longer *rejects* on stock: see "Selling into negative stock" below
3. Calculates totals
...
- **A movement that *removes* stock may not push it below zero → 400** on `cantidad` (`"El movimiento dejaría el stock en -94. Disponible de 'Pan': 5."`). **A movement that *adds* stock is always allowed**, even when the result is still negative. The guard is deliberately **directional** (`if cantidad < 0 and resultante < 0`): once sales were allowed to oversell, the old result-only rule meant a product sitting at `-10` rejected a partial `ENTRADA` of `+5` — the shop could fall into the hole but only climb out in one jump covering the whole debt. A negative `AJUSTE` keeps the old treatment: a sale is a fact that already happened, a manual adjustment is somebody typing.
...
- **Selling into negative stock — a sale is NEVER refused for lack of stock.** Overselling drives `stock_actual` below zero on purpose: the shop really handed the goods over, and blocking the till until somebody records an arrival stops the business from trading. The negative *is* the reminder that an `ENTRADA` is owed. Decided by the owner; see `ADR-SALES-20260816-stock-negativo-permitido` in the cerebro. Consequences worth knowing:
  - `POST /api/sales/` answers **201** where it used to answer 400, and its response carries an extra **`stock_negativo`** key: the products *of that sale* that ended below zero (`id`, `nombre`, `stock_actual`). It is added in `SaleViewSet.create`, **not** in `SaleSerializer`, because on `list`/`retrieve` the same key would be a stale photo of a stock that has moved since. The POS prints it on the success modal — that is the only moment the cashier learns a `ENTRADA` is owed.
  - `Product.stock_actual` therefore has **no `MinValueValidator`** (unlike `stock_minimo` and every price). It never blocked this path anyway — sales write through `F()` + `.update()`, which never call `full_clean()` — it only made the model lie and locked an admin out of `/admin/` for any product sitting at `-10`. Migration `products/0004`, an `AlterField` with no DB change: the column was already a plain `IntegerField` with no `CHECK`.
  - The inventory guard had to become directional in the same change, or the hole had no exit — see the `InventoryMovementSerializer` bullet above.
- **`create()` prices and decrements line by line** — `total += precio * item["cantidad"]` and one `F()` update per item. The old per-product *sum* in `_resolve_products()` existed only to be compared against the stock; with the comparison gone it had no reader, so it is now just a `set` of distinct ids for the `id__in` and the "not found or inactive" check. Two lines of 3 therefore decrement twice, which under this ADR is the right answer rather than the bug it used to be. Duplicate lines are still **not** merged into one `SaleItem` — the receipt shows what the client sent.
- **`cantidad` is capped at 10.000 per line.** The stock check used to be the de-facto ceiling (you could not sell 30.000 of something that had 20); with it gone, a fat-fingered quantity could overflow `Sale.total`'s `numeric(10,2)` into a 500 instead of a clean 400.
- **`stock_actual` is `read_only` on `ProductSerializer`.** Stock moves only through `InventoryMovement` — the one path with `F()`, a row lock and an audit trail. This became load-bearing when the model validator came off: DRF derives a field's `min_value` from the model validators, so `PATCH {"stock_actual": -9999}` briefly started answering 200.
- Stock is only tracked for `CON_CODIGO` products; `SIN_CODIGO` items never appear in `stock_negativo`.
