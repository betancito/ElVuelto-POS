...
A third folder, `el_vuelto_desktop/`, holds the **Windows desktop wrapper** for the cashier station: an Electron shell around the same web app whose reason to exist is **silent receipt printing** (a browser cannot print without the OS dialog). `python3 build.py` asks for environment + tenant slug and emits `ElVuelto-<slug>.exe`; the cashier picks the printer on first launch. It is generated by hand today — there is no download module in the tenant admin and no code signing yet. See `el_vuelto_desktop/README.md`.
...
./scripts/manage-docker.sh up dev         # prints the LAN URL when it finishes
...
- **`:8000`** — the backend straight through, for curl/Postman/Swagger.

Same-origin is the whole point: `apiBase.ts` calls the API at the **relative**
path `/api`, so there is no CORS preflight and no host:port in the bundle — the
same build works at `localhost`, at a LAN IP, and at a domain later.

**There is no `db` service.** Postgres already runs as a container outside this
stack and holds the live `elvuelto` database; the backend reaches it via
`DB_HOST=host.docker.internal`. For a deployment, change that one variable.

Two traps worth knowing before touching this:

- `proxy_set_header Host $http_host` (in `docker/nginx/proxy_common.conf`),
  **never `$host`** — `$host` drops the port, `request.get_host()` stops
  matching the browser's `Origin`, and every POST from the LAN dies with a 403
  CSRF error that no `CSRF_TRUSTED_ORIGINS` entry can fix.
- Dev servers must bind `0.0.0.0`, never `127.0.0.1`, or nginx answers 502.
...
- **Receipts**: generated on the **frontend**. Thermal geometry is a two-file contract and both sides
must agree: `main.js` prints with `marginType: 'none'`, so `generateReceipt.ts` MUST keep
`@page { margin: 0 }` — if both set margins they **add up** and the receipt prints shifted down and
right, clipped on a real 80mm roll (fixed 2026-08-27 against actual hardware). The content is 70mm
**centred** in the 80mm page because a "80mm" thermal only marks ~70-72mm; the rest is dead zone.
Page height is **measured** from `document.body.getBoundingClientRect().height` — never
`documentElement.scrollHeight`, which floors at the viewport. `generateReceipt.ts` builds the 80mm thermal layout as an HTML string; `printReceipt.ts` sends it either to the Electron wrapper's silent-print bridge or to `window.print()`. **Neither uses jsPDF** — the only jspdf importer in the repo is `downloadCredentials.ts`, which makes the credentials PDF. The backend has no printing dependency (`python-escpos` was removed).
