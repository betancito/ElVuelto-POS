...
> [!warning] Auditoría del 2026-09-13 — la última fila está 🟢 pero **no tiene RUN**
> La fila de [[TENANCY-20260830-factura-electronica-por-tenant]] se declara `🟢 corrido-ok` y su columna
> **Reporte apunta al ADR**, no a un `corridas/RUN-...`. No existe ningún `RUN-20260830-*` en todo el
> vault. [[GOBERNANZA]] §9 dice que el reporte extenso va en `corridas/`, y el veredicto menciona
> *"7 casos contra servidor real"* que **hoy son inauditables**: no quedó registrado cuáles fueron, con
> qué payloads ni qué devolvieron.
>
> El **fondo** de la feature sí se re-verificó contra código en el PASO 0 del 2026-09-13 y **aguanta**:
> las 6 preguntas de control dieron a favor del ADR (`default=False` en modelo y migración, solo el
> super admin la escribe, los 3 flujos de login del backend la mandan, no se aplica en caliente, el
> `overflow-wrap` está y también en el bundle desplegado). Lo que falta es la **traza**, no el trabajo.
> Ver [[2026-09-13-planner-paso0-resync]].
